/**
 * Forex Paper Trading Manager
 *
 * Manages N forex strategies running simultaneously in paper trading mode.
 * Key differences from Stock paper trading:
 * - Market hours: 24/5 (Sunday 22:00 UTC to Friday 22:00 UTC)
 * - Tick every 5 minutes
 * - Pairs instead of tickers (EURUSD, GBPUSD, etc.)
 * - MarketSnapshot uses priceChange24hPct from quote delta
 * - Persistence in Supabase (forex_paper_sessions table)
 * - Auto-start for L1-passing strategies
 * - Cooldown 8 hours (between stocks 12h and crypto 6h)
 */
...
export const FOREX_L1_STRATEGY_CODES = [
...
export const FOREX_L1_DEFAULT_CAPITAL = 100;

// ============================================================================
// Auto-rotation constants
// ============================================================================

/** Cooldown period after circuit breaker for forex sessions (8 hours) */
export const FOREX_COOLDOWN_HOURS = 8;

/** Maximum automatic rotations before permanent stop */
export const FOREX_MAX_AUTO_ROTATIONS = 3;

// ============================================================================
// Types
// ============================================================================

export interface ForexPaperSession {
  sessionId: string;
  strategy: ParsedStrategy;
  strategySeed: ForexStrategySeed;
...
export interface ForexTickResult {
  sessionId: string;
  strategyCode: string;
...
export interface ForexSessionOverview {
  sessionId: string;
  strategyCode: string;
  strategyName: string;
  status: string;
...
export interface ForexPaperOverview {
...
export class ForexPaperTradingManager {
  private sessions: Map<string, ForexPaperSession> = new Map();
  private adapter: ForexDataAdapter | null = null;

  /** Initialize Twelve Data adapter for forex market data */
  initializeAdapter(config: ForexDataAdapterConfig): void {
    if (!this.adapter) {
      this.adapter = new ForexDataAdapter(config);
    }
  }
   ...
   */
  async autoStartL1Sessions(initialCapital = FOREX_L1_DEFAULT_CAPITAL): Promise<string[]> {
    ...
    for (const code of FOREX_L1_STRATEGY_CODES) {
      if (activeStrategyCodes.has(code)) continue;

      const seed = FOREX_STRATEGY_MAP[code];
      if (!seed) continue;
  ...
  /** Start a paper trading session for a forex strategy */
  async startSession(seed: ForexStrategySeed, initialCapital = 1000): Promise<ForexPaperSession> {
    const strategy = parseForexSeed(seed);
    ...
    const config: ExecutorConfig = {
      ...
      maxOpenPositions: 5,
      slippagePct: 0.15, // Forex has tightest spreads
    ...
    if (error || !row) {
      throw new Error(`Errore creazione sessione forex: ${error?.message ?? 'sconosciuto'}`);
    }

    const session: ForexPaperSession = {
      sessionId: row.id,
      strategy,
      strategySeed: seed,
      executor,
  ...
  async tick(): Promise<ForexTickResult[]> {
    if (!this.adapter) {
    ...
    console.log(`[ForexManager] tick: ${this.sessions.size} sessions in memory`);

    const snapshots = await this.fetchForexSnapshots();
    console.log(`[ForexManager] tick: ${snapshots.length} market snapshots fetched`);

    const results: ForexTickResult[] = [];
  ...
  tickWithSnapshots(snapshots: MarketSnapshot[]): ForexTickResult[] {
    const results: ForexTickResult[] = [];
    ...
    for (const [sessionId, session] of this.sessions) {
      ...
      const filteredSnapshots = snapshots.filter((s) =>
        session.pairs.some((p) => s.marketId.includes(p))
      );
  ...
  getActiveSessions(): ForexPaperSession[] {
    return [...this.sessions.values()].filter((s) => s.status === 'running');
  }

  /** Get session by ID */
  getSession(sessionId: string): ForexPaperSession | undefined {
    return this.sessions.get(sessionId);
  }

  /** Get overview from in-memory sessions (for testing) */
  getOverview(): ForexPaperOverview {
    const sessions = [...this.sessions.values()];
    let totalCapital = 0;
    let totalPnl = 0;

    const sessionSummaries: ForexSessionOverview[] = sessions.map((s) => {
  ...
  async getOverviewFromDb(): Promise<ForexPaperOverview> {
    const db = createUntypedAdminClient();

    const { data: rows, error } = await db
      .from('forex_paper_sessions')
      .select('id, strategy_code, strategy_name, initial_capital, current_capital, total_pnl, total_pnl_pct, max_drawdown_pct, total_ticks, pairs, started_at, last_tick_at, status, is_circuit_broken, stopped_at')
    ...
    const sessions: ForexSessionOverview[] = rows.map((r: Record<string, unknown>) => {
  ...
  async startRotatedSession(
    ...
    initialCapital = FOREX_L1_DEFAULT_CAPITAL,
  ): Promise<string> {
    const db = createUntypedAdminClient();
    ...
    // Create new session
    const seed = FOREX_STRATEGY_MAP[strategyCode];
    ...
    if (error || !row) {
      throw new Error(`Errore creazione sessione ruotata forex: ${error?.message ?? 'sconosciuto'}`);
    }
  ...
  private async tickSession(
    sessionId: string,
    session: ForexPaperSession,
    snapshots: MarketSnapshot[],
  ): Promise<ForexTickResult> {
    ...
    try {
      ...
      for (const pos of snapshot.openPositions) {
        if (!preOpenIds.has(pos.id)) {
          const symbol = pos.marketName || pos.marketId.replace('FX:', '');

          await db.from('forex_paper_positions').insert({
          ...
          await db.from('forex_paper_trades').insert({
            session_id: sessionId,
            symbol,
            action: 'enter',
      ...
      for (const closed of newClosedPositions) {
        const symbol = closed.marketName || closed.marketId.replace('FX:', '');

        await db
          .from('forex_paper_positions')
          .update({
            status: 'closed',
      ...
      if (result.circuitBroken) {
        const cooldownUntil = new Date(Date.now() + FOREX_COOLDOWN_HOURS * 60 * 60 * 1000).toISOString();
  ...
  async loadActiveSessions(): Promise<number> {
    ...
    if (error) {
      console.error('[ForexManager] loadActiveSessions DB error:', error.message);
      return 0;
    }

    console.log(`[ForexManager] loadActiveSessions: ${rows?.length ?? 0} running sessions in DB, ${this.sessions.size} in memory`);
    ...
    for (const row of rows) {
      try {
        ...
        const seed = FOREX_STRATEGY_MAP[row.strategy_code];
        if (!seed) {
          console.warn(`[ForexManager] Strategy ${row.strategy_code} not found in FOREX_STRATEGY_MAP — skipping session ${row.id}`);
          continue;
        }
        ...
        console.error(`[ForexManager] Error loading session ${row.strategy_code} (${row.id}):`, err instanceof Error ? err.message : err);
      }
    }

    console.log(`[ForexManager] loadActiveSessions: loaded ${loaded} new sessions, total in memory: ${this.sessions.size}`);
    return loaded;
  }

  /** Fetch market snapshots via Twelve Data batch endpoint */
  private async fetchForexSnapshots(): Promise<MarketSnapshot[]> {
    ...
    try {
      ...
      return quotes.map((quote) => {
        // Calculate price change from previousClose if available
        const prevClose = quote.previousClose ?? 0;
        const mid = (quote.bid + quote.ask) / 2;
        const priceChangePct = prevClose > 0
          ? ((mid - prevClose) / prevClose) * 100
          : 0;

        return {
          marketId: `FX:${quote.symbol}`,
          name: quote.symbol,
          price: mid,
          volume24hUsd: quote.volume24h,
          totalVolumeUsd: quote.volume24h * 8,
          expiryDate: null,
          hasCatalyst: false,
      ...
      console.error('[ForexManager] fetchForexSnapshots failed:', err instanceof Error ? err.message : err);
      return [];
    }
  }
}
...
function parseForexSeed(seed: ForexStrategySeed): ParsedStrategy {
...
let forexManagerInstance: ForexPaperTradingManager | null = null;

export function getForexPaperTradingManager(): ForexPaperTradingManager {
  if (!forexManagerInstance) {
    forexManagerInstance = new ForexPaperTradingManager();
  }
  return forexManagerInstance;
}
