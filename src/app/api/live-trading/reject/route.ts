/**
 * POST /api/live-trading/reject — Reject a pending trade from web.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const resolved = resolveApproval(tradeId, false);
    if (!resolved) {
      return NextResponse.json({ error: 'Trade non trovato o gia\' gestito' }, { status: 404 });
    }

    return NextResponse.json({ ok: true, action: 'rejected', tradeId });
  } catch (error) {
