/**
 * GET /api/cron/stock-tick
 *
 * Vercel Cron — every 5 minutes.
 * Executes a tick on all active stock paper trading sessions:
 * - SKIPS outside NYSE market hours (14:30-21:00 UTC, weekdays only)
 * - Auto-starts L1-passing strategies if no sessions are active
 * - Fetches latest stock data from Finnhub
 * - Evaluates stock strategies against current markets
...
import { getStockPaperTradingManager, StockTickResult, STOCK_L1_STRATEGY_CODES, STOCK_L1_DEFAULT_CAPITAL } from '@/core/paper-trading/stock-manager';
import { STOCK_STRATEGY_MAP } from '@/core/strategies/stock-strategies';
import { isMarketOpen } from '@/plugins/stocks/adapter';
...
export async function GET(request: NextRequest) {
  ...
  // --- Market hours check ---
  if (!isMarketOpen()) {
    console.log('[Cron/stock-tick] Market closed — skipping tick');
    return NextResponse.json({
      ok: true,
      skipped: true,
      reason: 'Market closed (NYSE 14:30-21:00 UTC, weekdays only)',
    });
  }
  ...
  try {
    ...
    if (activeBefore === 0) {
      ...
      const { data: existingSessions } = await db
        ...
        .in('strategy_code', [...STOCK_L1_STRATEGY_CODES]);
      ...
      const strategiesWithNoSession = STOCK_L1_STRATEGY_CODES.filter(
        (code) => !existingByCode.has(code),
      );
      const strategiesOnlyStopped = STOCK_L1_STRATEGY_CODES.filter(
      ...
      if (canAutoStart) {
        ...
        if (autoStarted.length > 0) {
          try {
            ...
            await client.sendMessage(
              ELIO_CHAT_ID,
              `\uD83D\uDCC8 <b>Stock Paper Trading Auto-Start</b>\n\n` +
        ...
        console.log('[Cron/stock-tick] Auto-start skipped — existing paused/stopped sessions:', autoStartSkipped);
      }
    }
    ...
    await sendStockTickNotifications(results);

    // Performance warnings
    await checkStockPerformanceWarnings();

    // Reset failure counter on success
    consecutiveFailures = 0;

    console.log('[Cron/stock-tick]', JSON.stringify(summary));

    return NextResponse.json({ ok: true, summary });
  } catch (error) {
    consecutiveFailures++;
    const message = error instanceof Error ? error.message : 'Errore sconosciuto';
    console.error(`[Cron/stock-tick] ERRORE (failure #${consecutiveFailures}):`, message);
    ...
    if (shouldNotify) {
      ...
      try {
        ...
        await client.sendMessage(
          ELIO_CHAT_ID,
          `${urgency} <b>Cron Stock Tick Fallito</b>\n\n` +
...
async function sendStockTickNotifications(results: StockTickResult[]): Promise<void> {
  const client = getTelegramClient();

  for (const result of results) {
    // Circuit breaker alert
    if (result.circuitBroken) {
      const { currentDrawdown, maxDrawdown, reason } = await fetchStockCircuitBreakerDetails(result.sessionId, result.strategyCode);

      await client.sendCircuitBreakerAlert(ELIO_CHAT_ID, {
        strategyId: result.sessionId,
        strategyName: `[STOCK] ${result.strategyCode}`,
        currentDrawdown,
        maxDrawdown,
        action: `Sessione stock messa in pausa \u2014 ${reason}`,
    ...
    if (result.positionsOpened > 0 || result.positionsClosed > 0) {
      const lines: string[] = [
        `\uD83D\uDD04 <b>Stock Tick ${result.strategyCode}</b>`,
        '',
      ];
      ...
      lines.push(`\uD83D\uDCCA Ticker analizzati: ${result.tickersEvaluated}`);
...
async function fetchStockCircuitBreakerDetails(
  sessionId: string,
  strategyCode: string,
  ...
  try {
    ...
    if (session) {
      ...
      const seed = STOCK_STRATEGY_MAP[session.strategy_code || strategyCode];
...
async function checkStockPerformanceWarnings(): Promise<void> {
  try {
    const db = createUntypedAdminClient();
    ...
    for (const session of sessions) {
      const seed = STOCK_STRATEGY_MAP[session.strategy_code];
      if (!seed) continue;
    ...
    console.error('[Cron/stock-tick] Performance warning check failed:', err);
  }
}
